...
public class MechanicalFryerRenderer extends ShaftRenderer<MechanicalFryerEntity> {
    ...
    protected void renderSafe(MechanicalFryerEntity fryerEntity, float partialTicks, PoseStack poseStack, MultiBufferSource bufferSource, int light, int overlay) {
        ...
        ItemStack inputStack = getFirstNonEmptyStack(fryerEntity.inputInv);
        ItemStack outputStack = getFirstNonEmptyStack(fryerEntity.outputInv);
    ...
    private ItemStack getFirstNonEmptyStack(Storage<ItemVariant> storage) {
        for (StorageView<ItemVariant> view : storage) {
            if (!view.isResourceBlank() && view.getAmount() > 0) {
                // Convert the amount to an integer assuming the amount fits into an ItemStack. This may need adjusting depending on the specific use case.
