...
public class OiledUpEffect extends MobEffect {
    ...
    private void maybeFloatInWater(Player player) {
        if (player.isInWater() && !player.level().getBlockState(player.blockPosition().above()).isAir()) {
    ...
    private void maybeFallOffLadder(Player player) {
        ...
        boolean isOnLadder = player.level().getBlockState(player.blockPosition()).is(BlockTags.CLIMBABLE) ||
                player.level().getBlockState(player.blockPosition().above()).is(BlockTags.CLIMBABLE);
